...
public:
    AttendanceSession(string code, string dt, string st, string et, string dur)
        : courseCode(code), date(dt), startTime(st), endTime(et), duration(dur) {}
    ...
    void saveToFile() const {
        ...
        if (outFile.is_open()) {
            outFile << "Course: " << courseCode << ", Date: " << date
                    << ", Start Time: " << startTime <<", End Time: "<< endTime << ", Duration: " << duration << endl;
...
void markAttendance(vector<Student>& students) {
    ...
    string code, start, end, duration;
    ...
    AttendanceSession session(code, date, start, end, duration);
